...
PATH_BSPWM_GNOME_SYSTEMD_CONF = $(PREFIX)/lib/systemd/user/gnome-session@bspwm-gnome.target.d/bspwm-gnome.session.conf
...
install:
	...
	$(INSTALL) -m0644 -D session/bspwm-gnome.session.conf $(PATH_BSPWM_GNOME_SYSTEMD_CONF)
...
uninstall:
	...
	rm -f $(PATH_BSPWM_GNOME_SYSTEMD_CONF)
	rm -f $(PATH_BSPWM_GNOME_XSESSION)
	rm -f $(PATH_GNOME_SESSION_BSPWM)
	rmdir --ignore-fail-on-non-empty $(dir $(PATH_BSPWM_GNOME_SYSTEMD_CONF))
